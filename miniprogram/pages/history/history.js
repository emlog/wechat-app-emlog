...
Page({
    ...
    loadArticleByPage() {
        ...
        wx.request({
            url: app.globalData.baseUrl + 'article_list&page=' + page + '&count=' + size + '&sort=' + sort1 + '&sort=' + sort2,
            method: 'GET',
            success: function (res) {
                wx.hideLoading()
    ...
    loadCategories() {
        const that = this;
        wx.request({
            url: app.globalData.baseUrl + 'sort_list',
            method: 'GET',
            success: function (res) {
                if (res.data.code == 0) {
                    that.setData({
                        allCategories: res.data.data.sorts,
                    })
                }
            },
            fail: function (res) {
    ...
    loadPostsBySlug(slug) {
        ...
        const size = that.data.pageSize;
        wx.request({
            url: app.globalData.baseUrl + 'article_list&sort_id=' + slug + '&page=' + page + '&size=' + size,
            method: 'GET',
            success: function (res) {
                wx.hideLoading()
                if (res.data.code == 0) {
